...
const BookingConflictsPage: React.FC = () => {
  ...
  // Функция для перевода примечаний
  const translateResolutionNotes = (notes: string) => {
    if (notes === 'Test resolution') {
      return t('bookingConflicts.messages.testResolution');
    }
    return notes;
  };
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                  ...
                  <Box sx={{ flexGrow: 1 }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                      ...
                      <Chip
                        label={t(`bookingConflicts.status.${conflict.status}`)}
                    ...
                    <Grid container spacing={2}>
                      <Grid item xs={12} md={6}>
                        ...
                        <Typography variant="body2">
                          ...
                          {t('bookingConflicts.bookingInfo.time')}: {conflict.booking.start_time ? new Date(conflict.booking.start_time).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }) : t('bookingConflicts.bookingInfo.notSpecifiedTime')}<br />
                    ...
                    {conflict.resolution_notes && (
                      <Alert severity="info" sx={{ mt: 2 }}>
                        <Typography variant="body2">
                          <strong>{t('bookingConflicts.dialogs.resolve.notes')}:</strong> {translateResolutionNotes(conflict.resolution_notes)}
                        </Typography>
                      </Alert>
                    )}
                  </Box>
